...
---
Task ID: referral-v3-payout-rules
Agent: Main Agent
Task: Implement the owner's V3 referral payout spec — $50 on the referred driver's 5th paid delivery within 30 days of signup (expires unpaid, no partial payout), one-time $10 business / $5 residential first-delivery rewards with a rolling $300 cap, unified role matrix (any user can refer anyone), new-user-only enforcement, and live progress (X of 5, days remaining) for both sides

Work Log:
- Audited the existing engine first (found: expiry machinery fully built but dead — applyReferral hardcoded expiresAt=null; personal customers blocked from referring drivers; business customers rejected as referees; $5/$10 keyed to REFERRER type and paid PER delivery (spec wants REFERRED type, one-time); no cap; getMyReferrals missing completedPaidDeliveries so the wallet badge always showed "Delivery 0 of 30"; driver copy read requiredDeliveries (30) instead of perDeliveryBonusTriggerCount (5)).
- Prisma: new EnumReferralCategory (DRIVER_REFERRAL / BUSINESS_REFERRAL / RESIDENTIAL_REFERRAL) + Referral.category snapshot column (stamped at apply time so a later customerType upgrade can't rewrite history; legacy null-category rows fall back to residential semantics). Migration 20260904090000_referral_category with backfill (caught + fixed a self-written AS VALUE → AS ENUM bug by diffing against the repo's own migration convention before push).
- appSetting config: defaults flipped — payoutModel TIERED→PER_DELIVERY, referredGetsReward true→false (V3 pays referrers only); new admin-tunable fields referralWindowDays=30, businessReferralAmountCents=1000, residentialReferralAmountCents=500, businessReferralRollingCapCents=30000 (response DTO + update DTO + normalizer + update merge).
- applyReferral: unified matrix (removed the personal-customer block — any user can refer a driver); "already in the system" guard (reject codes from APPROVED drivers / approvedAt set — pre-approval application window, decision D2); REAL expiry — expiresAt = driver.createdAt + referralWindowDays (clock anchor = account creation, documented decision D1); category stamped DRIVER_REFERRAL.
- applyCustomerReferral: business customers CAN now be referred (category BUSINESS_REFERRAL vs RESIDENTIAL_REFERRAL by customerType snapshot); expiresAt stays null (no window for customer referrals per spec).
- Trigger engine: customer-referral path rewritten to ONE-TIME first-delivery semantics — race-safe atomic claim (updateMany where completedPaidDeliveries=0), amount by category ($10 business / $5 residential), payout vehicle by referrer type (DriverPayout vs ReferralCredit), referral goes terminal REWARD_PAID after the first delivery (payout made or forfeited); rolling 30-day cap for BUSINESS_REFERRAL (usage summed from ReferralCredit joins + DriverPayout failureMessage attribution, new key format PER_DELIVERY:<referralId>:<deliveryId>); overflow FORFEITED not deferred (decision D3). CRITICAL FIX: removed referredGetsReward from the $50-to-referrer gate — the D4 default flip would otherwise have silently killed the referrer's $50.
- Scheduler: calendar-window gate removed — expiry sweep is unconditional (per-referee windows live on each row now); AppSettingService dependency dropped.
- Progress APIs: getMyReferrals + completedPaidDeliveries/category/payoutModel/daysRemaining; getMyReferralStats + triggerCount/referralWindowDays/payoutModel; getMyCustomerReferrals + category/daysRemaining; getMyCustomerReferralStats + perReferrals progress list + businessReferralCap/UsedCents + new amounts; NEW GET /referrals/my-referred-status (referred side: "Referred by X · N of 5 · D days left", expiredUnpaid flag).
- Frontend: driver-wallet badge now "Delivery X of 5 · Nd left" with a red EXPIRED state (was always "0 of 30"); new referred-side progress banner card (my-referred-status); ReferralCodeCard driver copy fixed to perDeliveryBonusTriggerCount + window-days wording, customer copy rewritten to one-time semantics, cap meter + per-referral progress list added (dealer dashboard); admin-referral-program page gains the 4 V3 fields (window days, business $, cap $, residential $) with documented defaults.
- Tests: rewrote 3 stale per-delivery expectations to V3 semantics; added 3 new V3 tests (business $10 one-time, cap forfeit + still-terminal, residential never cap-checked). src/appSetting + src/referral: 62/62 PASS.
- Verification: backend tsc → only the 4 pre-existing errors (upload.module, platform-socket.io, 2 spec arg-count), 0 new; frontend scoped tsc (tsconfig.fe-check.json) 144 baseline == 144 after, 0 new; vite build passes; lockfile churn reverted per convention. No live PG in this sandbox — migration SQL manually validated against the repo's own migration patterns (AS ENUM) instead of a fresh-DB migrate deploy run.
