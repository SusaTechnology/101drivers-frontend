...
// Mock filter options (kept for UI)
const MOCK_JOBS = {
  inboxCount: 3, // will be replaced if we have real inbox count later
  distanceOptions: [
  ...
  sortOptions: [
    ...
    { value: 'shortest', label: 'Shortest miles' },
...
const bottomNavItems = [
  ...
  { href: '/driver-inbox', label: 'Inbox', icon: Inbox, badge: 3 },
...
export default function DriverDashboardPage() {
  ...
  const {
    ...
    apiEndPoint: `${import.meta.env.VITE_API_URL}/api/deliveryRequests/driver/feed/${driverId}`,
    noFilter: true,
  ...
  const handleApplyFilters = (data: FilterFormData) => {
    ...
    // In a real app, you would refetch with filters
  }
          ...
          <Card className="border-slate-200 dark:border-slate-800 shadow-lg">
            <CardContent className="p-5 sm:p-6">
              <form onSubmit={filterForm.handleSubmit(handleApplyFilters)}>
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                  ...
                  <div className="sm:col-span-3 space-y-2">
                    ...
                    <Select
                      onValueChange={(value) => filterForm.setValue('distance', value)}
                      defaultValue="25"
                    >
                      ...
                      <SelectContent>
                        {MOCK_JOBS.distanceOptions.map((option) => (
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    <Select
                      onValueChange={(value) => filterForm.setValue('when', value)}
                      defaultValue="today"
                    >
                      <SelectTrigger className="h-12 rounded-2xl border-slate-200 dark:border-slate-700 dark:bg-slate-800/40">
                      ...
                      <SelectContent>
                        {MOCK_JOBS.whenOptions.map((option) => (
                          <SelectItem key={option.value} value={option.value}>
                            {option.label}
                          </SelectItem>
                        ))}
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {MOCK_JOBS.typeOptions.map((option) => (
                          <SelectItem key={option.value} value={option.value}>
                            {option.label}
                          </SelectItem>
                        ))}
                ...
                <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Badge variant="outline" className="chip bg-amber-50 dark:bg-amber-900/10 border-amber-100 dark:border-amber-900/30 text-amber-900 dark:text-amber-200">
                      <BoltIcon className="w-3.5 h-3.5 text-amber-500 mr-1" />
                      Urgent bonus
                    </Badge>
                  </div>
              ...
              <p className="mt-4 text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
                Prototype filters only. In production, results respect your service-area preferences (district/city/ZIP/radius) and alert rules.
              </p>
            </CardContent>
          </Card>
        </section>
          ...
          <div className="flex items-end justify-between gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              >
                ...
                <SelectContent>
                  {MOCK_JOBS.sortOptions.map((option) => (
                    <SelectItem key={option.value} value={option.value}>
                      {option.label}
                    </SelectItem>
                  ))}
              ...
              <div className="p-5 sm:p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                    {jobs.length} job{jobs.length !== 1 ? 's' : ''} available. In production, booking is first-come/first-served; if another driver books it first, you'll see "Already booked".
                  </p>
                  <Link
                    to="/driver-preferences"
            ...
            <CardContent className="p-5 sm:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                    If an issue happens, report it and Ops will assist. All actions are logged in production.
                  </p>
                </div>
