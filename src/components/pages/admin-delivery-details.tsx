...
export default function AdminDeliveryDetailsPage({ deliveryId }: { deliveryId: string }) {
  ...
  // Whether the admin chose to apply the configured close penalty. Default: false.
      ...
      onSuccess: () => {
        if (delivery?.lockedInAt && delivery?.lockInBaseFee) {
          ...
          toast.success('Delivery cancelled with penalty', {
            description: `$${(penaltyPreview.penaltyAmountDollars ?? 0).toFixed(2)} penalty applied to the customer and paid to the driver.`,
          });
        } else {
        ...
        <AlertDialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            {/* Close penalty preview — fetched when the dialog opens.
                ...
                whether to apply the configured penalty fee to the customer
