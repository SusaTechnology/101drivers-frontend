...
// ------------------------------------------------------------------
// VEHICLE DROPDOWN OPTIONS (synced with dealer-create-delivery.tsx)
// ------------------------------------------------------------------
const MAKE_OPTIONS = [
  'Toyota',
  'Honda',
...
const confirmDelete = () => {
  ...
  // Saved vehicles - using useQuery directly to avoid pagination params
  const {
    data: vehicles = [],
  ...
  const handleVehicleSubmit = () => {
    ...
    if (vehicleForm.make === 'Other' && !vehicleForm.makeOther.trim()) {
      toast.error('Please enter the make')
      return
    }
    ...
    if (vehicleForm.model === 'Other' && !vehicleForm.modelOther.trim()) {
    ...
    if (vehicleForm.color === 'Other' && !vehicleForm.colorOther.trim()) {
    ...
    const finalMake = vehicleForm.make === 'Other' ? vehicleForm.makeOther : vehicleForm.make
    const finalModel = vehicleForm.model === 'Other' ? vehicleForm.modelOther : vehicleForm.model
    const finalColor = vehicleForm.color === 'Other' ? vehicleForm.colorOther : vehicleForm.color
  ...
  const handleEditVehicle = (v: SavedVehicle) => {
    // Check if the values are in our dropdown options, otherwise set to "Other"
    const makeValue = MAKE_OPTIONS.includes(v.make as any) ? v.make : 'Other'
    const modelValue = MODEL_OPTIONS.includes(v.model as any) ? v.model : 'Other'
    const colorValue = COLOR_OPTIONS.includes(v.color as any) ? v.color : 'Other'
    
    setVehicleForm({
      make: makeValue,
      makeOther: makeValue === 'Other' ? v.make : '',
      model: modelValue,
      modelOther: modelValue === 'Other' ? v.model : '',
      color: colorValue,
      colorOther: colorValue === 'Other' ? v.color : '',
  ...
  const isPrivateCustomer = customer?.customerType === 'PRIVATE';
  const isBusinessCustomer = customer?.customerType === 'BUSINESS';
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden lg:inline-flex items-center gap-2 px-4 py-2 rounded-full bg-lime-500/10 text-slate-900 border border-lime-500/25 ml-2">
              ...
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-700">
                {isPrivateCustomer ? 'INDIVIDUAL' : 'BUSINESS'}
              </span>
            </div>
          </div>
      ...
      <main className="w-full max-w-[1440px] mx-auto px-6 lg:px-8 py-8 lg:py-10">
        ...
        <section className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-6">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              {/* Approval status - only for BUSINESS customers */}
              {isBusinessCustomer && (
              ...
              {/* Postpaid badge - only for BUSINESS customers with postpaid enabled */}
              {isBusinessCustomer && customer?.postpaidEnabled && (
                <Badge variant="secondary" className="gap-1 bg-sky-500/10 text-sky-700 border-sky-500/20">
                  <CreditCard className="h-3 w-3" />
                  Postpaid Enabled
                </Badge>
              )}
              {/* Private customers are prepaid only */}
              {isPrivateCustomer && (
                <Badge variant="secondary" className="gap-1 bg-lime-500/10 text-lime-700 border-lime-500/20">
                  <CreditCard className="h-3 w-3" />
                  Prepaid Only
                </Badge>
              )}
            ...
            <div>
              ...
              <p className="text-sm lg:text-base text-slate-600 dark:text-slate-400 max-w-3xl mt-2">
                {isPrivateCustomer 
                  ? 'Manage your contact details, saved vehicles, and account preferences from one screen.'
                  : 'Manage business profile, contact details, saved pickup addresses, saved vehicles, billing visibility, and account preferences from one screen.'
                }
              </p>
            </div>
          </div>
        ...
        <section className="mt-8 grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6 items-start">
          ...
          <div className="space-y-6">
            {/* Business profile - Only for BUSINESS customers */}
            {isBusinessCustomer && (
            ...
            {/* Saved pickup addresses - Only for BUSINESS customers */}
            {isBusinessCustomer && (
            <Card className="rounded-3xl border-slate-200 dark:border-slate-800 hover:shadow-xl transition-shadow">
              <CardHeader className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
              ...
              <CardContent className="space-y-4">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* Row 5: Coordinates (technical, read-only) */}
                    <div className="space-y-2">
                    ...
                    {/* Row 6: Place ID (technical, read-only, full width) */}
                    <div className="md:col-span-2 space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setVehicleForm({ ...vehicleForm, make: value, makeOther: '' })}
                      >
                      ...
                      {vehicleForm.make === 'Other' && (
                        <Input
                          ...
                          value={vehicleForm.makeOther}
                          onChange={(e) => setVehicleForm({ ...vehicleForm, makeOther: e.target.value })}
                          className="h-12 rounded-2xl"
                        />
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setVehicleForm({ ...vehicleForm, model: value, modelOther: '' })}
                      >
                        <SelectTrigger className="h-12 rounded-2xl">
                      ...
                      {vehicleForm.model === 'Other' && (
                        <Input
                          ...
                          value={vehicleForm.modelOther}
                          onChange={(e) => setVehicleForm({ ...vehicleForm, modelOther: e.target.value })}
                          className="h-12 rounded-2xl"
                        />
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setVehicleForm({ ...vehicleForm, color: value, colorOther: '' })}
                      >
                        <SelectTrigger className="h-12 rounded-2xl">
                      ...
                      {vehicleForm.color === 'Other' && (
                        <Input
                          ...
                          value={vehicleForm.colorOther}
                          onChange={(e) => setVehicleForm({ ...vehicleForm, colorOther: e.target.value })}
                          className="h-12 rounded-2xl"
                        />
                      )}
                    </div>
          ...
          <aside className="space-y-6 xl:sticky xl:top-28">
            ...
            <Card className="rounded-3xl border-slate-200 dark:border-slate-800">
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
                  ...
                  <div className="font-black text-slate-900 dark:text-white mt-2">
                    {customer?.postpaidEnabled ? 'Prepaid & Postpaid' : 'Prepaid only'}
                  </div>
                  <div className="text-slate-600 dark:text-slate-400 mt-1">
                    {customer?.postpaidEnabled 
                      ? 'Your account is authorized for postpaid billing.'
                      : 'Contact support to enable postpaid billing.'}
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
                  <div className="text-[11px] font-black uppercase tracking-widest text-slate-400">
                  ...
                  <div className="font-black text-slate-900 dark:text-white mt-2">Admin-controlled</div>
                  <div className="text-slate-600 dark:text-slate-400 mt-1">
                    Pricing and billing policy are managed by the admin team.
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-3xl border-slate-200 dark:border-slate-800">
              ...
              <CardContent>
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  Submit a support request for delivery issues, payment questions, cancellations, or account help.
                </p>
                <div className="mt-6 flex flex-col gap-3">
                  <Link
                    to="/dealer-support-request"
                    className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-full bg-lime-500 text-slate-950 hover:bg-lime-600 font-extrabold transition"
                  >
                  ...
                  <Link
                    ...
                    className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-full bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 font-bold transition"
                  >
                  ...
                  <Button
                    ...
                    onClick={() => window.location.href = 'mailto:support@101drivers.com?subject=Support Request'}
                  >
