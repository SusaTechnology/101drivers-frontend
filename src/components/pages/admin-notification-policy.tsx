  ...
  channels: [
    {
      ...
      description: "Required. Email-first notifications for all stakeholders.",
      icon: Mail,
      enabled: true,
      required: true,
    },
...
export default function AdminNotificationPolicyPage() {
  ...
  const [rateLimit, setRateLimit] = useState(
    DEFAULT_NOTIFICATION_POLICY.rateLimit.limit,
  );
  const [dedupeWindow, setDedupeWindow] = useState(
    DEFAULT_NOTIFICATION_POLICY.dedupeWindow.default,
  );
  const [channels, setChannels] = useState(DEFAULT_NOTIFICATION_POLICY.channels);
  const [eventRules, setEventRules] = useState(
    DEFAULT_NOTIFICATION_POLICY.eventRules,
  );
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200 dark:border-slate-800 bg-white/85 dark:bg-background-dark/80 backdrop-blur-md">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              to="/admin-config"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition text-sm font-extrabold text-slate-700 dark:text-slate-200"
            >
                  ...
                  <div className="p-5 rounded-3xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      ...
                      >
                        ...
                        <SelectContent>
                          {DEFAULT_NOTIFICATION_POLICY.dedupeWindow.options.map(
                            (option) => (
