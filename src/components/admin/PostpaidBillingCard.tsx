    ...
    <Card className={cn(
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          ...
          {/* Switch billing mode button */}
          {status.postpaidEnabled ? (
            <Button
              onClick={() => openSwitchDialog('PREPAID')}
              variant="outline"
            ...
            <Button
              onClick={() => openSwitchDialog('POSTPAID')}
              variant="outline"
        ...
        {/* ── Switch billing mode confirmation dialog ── */}
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                Switch to {switchTarget === 'PREPAID' ? 'Prepaid' : 'Postpaid'} Billing
              </DialogTitle>
              <DialogDescription className="text-sm text-slate-600 dark:text-slate-400 mt-2">
                Review the impact before switching.
              </DialogDescription>
            </DialogHeader>
            ...
            {eligibilityLoading ? (
              ...
              <div className="space-y-3">
                {/* Block reason */}
                {!switchEligibility.canSwitch && (
                  <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                    <div className="flex items-start gap-2">
                      <AlertTriangle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
                      <p className="text-xs text-red-700 dark:text-red-300 font-medium">
                        {switchEligibility.blockReason}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {switchEligibility.canSwitch && (
                  <>
                    {switchTarget === 'PREPAID' && (
                      <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                        <div className="flex items-start gap-2">
                          <Info className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                          <div className="text-xs text-amber-700 dark:text-amber-300 space-y-1">
                            <p className="font-bold">Switching to Prepaid — what happens:</p>
                            <p>• New deliveries will be charged immediately at creation (prepaid)</p>
                            <p>• The Stripe subscription will be cancelled at the end of the current billing period</p>
                            {switchEligibility.pendingDeliveryCount > 0 && (
                              <p>• <strong>{switchEligibility.pendingDeliveryCount} completed delivery(ies) (${switchEligibility.outstandingBalance.toFixed(2)})</strong> will still be billed via the current postpaid cycle — no money is lost</p>
                            )}
                      ...
                      <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
                            ...
                            <p>• The customer must have a saved card on file (✅ confirmed)</p>
                            <p>• A Stripe subscription will be created (or re-enabled)</p>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <p className="text-sm font-black text-slate-900 dark:text-white">${switchEligibility.outstandingBalance.toFixed(2)}</p>
                      </div>
                      <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        <p className="text-[10px] font-bold uppercase text-slate-400">Pending deliveries</p>
                        <p className="text-sm font-black text-slate-900 dark:text-white">{switchEligibility.pendingDeliveryCount}</p>
                      </div>
                      <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        <p className="text-[10px] font-bold uppercase text-slate-400">Failed charges</p>
                        <p className={`text-sm font-black ${switchEligibility.failedChargeCount > 0 ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}>{switchEligibility.failedChargeCount}</p>
                      </div>
                      <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        <p className="text-[10px] font-bold uppercase text-slate-400">Saved card</p>
                        <p className="text-sm font-black text-slate-900 dark:text-white">{switchEligibility.hasSavedPaymentMethod ? '✅ Yes' : '❌ No'}</p>
                      </div>
                    </div>
                  </>
                )}
