...
export function ResetPassword() {
  ...
  // Auto-fill email from URL when component mounts
  useEffect(() => {
    if (emailFromUrl) {
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className={cn(
                    "h-14 pl-12 pr-4 rounded-2xl border-slate-200 dark:border-slate-700 input-focus-ring text-sm",
                    emailFromUrl ? "bg-slate-100 dark:bg-slate-800/60 cursor-not-allowed" : "dark:bg-slate-800/40"
                  )}
                  disabled={resetPasswordMutation.isPending || !!emailFromUrl}
                  {...register("email")}
                />
                {emailFromUrl && (
