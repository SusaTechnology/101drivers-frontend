...
function PendingDriverRow({ driver }: { driver: DriverOperationsType['recentPendingDrivers'][0] }) {
  return (
    <Link
      ...
      className="block"
    >
      <div className="flex items-center gap-2 p-2 rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 hover:border-primary/30 transition-colors cursor-pointer">
        ...
        <div className="text-right shrink-0">
          <Badge variant="outline" className="text-[9px] font-bold bg-amber-100 text-amber-700 border-amber-200 px-1.5 py-0">
            Pending
          </Badge>
      ...
      <CardHeader className="p-3 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className="text-sm font-black">Driver Operations</CardTitle>
          ...
          <Link to="/admin-users" search={{ tab: 'drivers' }}>
            <Badge variant="outline" className="chip cursor-pointer hover:bg-primary/10 text-[10px] px-2 py-1">
              View All
      ...
      <CardContent className="p-3">
        ...
        <div className="grid grid-cols-4 gap-1.5 mb-3">
          <div className="text-center p-2 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
            ...
            <p className="text-[8px] font-bold text-green-600 uppercase">Approved</p>
          </div>
          <div className="text-center p-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
            ...
            <p className="text-[8px] font-bold text-amber-600 uppercase">Pending</p>
          </div>
          <div className="text-center p-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            <p className="text-lg font-black text-slate-600">
              {data?.suspendedDrivers ?? 0}
            </p>
            <p className="text-[8px] font-bold text-slate-500 uppercase">Suspended</p>
          </div>
          <div className="text-center p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            ...
            <p className="text-[8px] font-bold text-blue-600 uppercase">On Trip</p>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="space-y-1.5">
            {[1, 2].map((i) => (
              <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 dark:bg-slate-900">
          ...
          <div className="flex flex-col items-center justify-center py-4 text-center">
            <UserCheck className="w-6 h-6 text-green-300 mb-1" />
            <p className="text-xs text-slate-500">No pending drivers</p>
          </div>
        )}
      </CardContent>
    </Card>
