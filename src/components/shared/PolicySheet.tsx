...
import { Handshake, AlertTriangle, Shield, Info, Clock, Users, Database, Lock, UserCheck, Mail } from "lucide-react";

interface PolicySheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  type: "agreement" | "terms" | "privacy";
  // optional: customize the close button text and action
  onCloseAction?: () => void;
  closeLabel?: string;
  fromSignUp: boolean;
}
...
export default function PolicySheet({
  ...
  // default behavior: just close the sheet
  const handleClose = () => {
    onOpenChange(false);
    onCloseAction?.();
  };
        ...
        <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader className="pt-8 px-6">
            <SheetTitle className="text-2xl font-black text-slate-900 dark:text-white">
              {type === 'agreement' && 'Independent Driver Agreement'}
              {type === 'terms' && 'Terms of Service'}
              {type === 'privacy' && 'Privacy Policy'}
            </SheetTitle>
            <SheetDescription className="text-sm text-slate-500 dark:text-slate-400">
              {type === 'agreement' && 'Effective: April 1, 2026'}
              {type === 'terms' && 'Effective date: March 2026'}
              {type === 'privacy' && 'Last updated: March 2026'}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="px-6 pb-10 space-y-4 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
            ...
            >
              {fromSignUp?"Go Back to Sign Up":"Go Back"}
            </button>
          </div>
        </SheetContent>
      </Sheet> 
